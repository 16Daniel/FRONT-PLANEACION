...
@Component({
  selector: 'app-asignar-proveedores',
  standalone: true,
  imports: [
    CommonModule,
    SkeletonModule,
  ...
  templateUrl: './AsignarProveedores.component.html',
})
export default class AsignarProveedoresComponent {
  ...
  { 
    this.confirmationService.confirm({
      ...
      accept: () => {
        this.loading = true;
        this.apiserv.deleteAsignaciones(JSON.stringify(this.registrosseleccionados)).subscribe({
          next: data => {
            this.getAsignaciones(); 
  ...
  saveData()
  {
    ...
    this.apiserv.guardarAsignacioones(this.selectedProv,this.selectedUSer,JSON.stringify(data)).subscribe({
      next: data => {
  ...
  getusuarios()
{
 ...
 getAsignaciones()
 {
    this.apiserv.getAsignaiones().subscribe({
     next: data => {
